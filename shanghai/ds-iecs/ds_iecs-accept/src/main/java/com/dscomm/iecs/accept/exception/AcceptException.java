...
public class AcceptException extends UserInterfaceException {
    ...
    public enum AccetpErrors implements UserInterfaceError {
        ...
        FIND_VEHICLE_DISPATCH_DISTRIBUTION_FAIL("车辆调派分布统计失败"),
        ...
        SAVE_UNIFORM_ADDRESS_FAIL("同一地址库保存失败"),
        GET_UNIFORM_ADDRESS_FAIL("根据警情id获取统一地址库id失败"),
        FIND_FAILED_FEEDBACK_FAIL("查询反馈失败的警情失败"),
        FIND_RELIEF_INCIDENTS_FAIL("获取救济警情失败"),
        FEEDBACK_TASK_FAIL("定期执行业务反馈失败"),
        WRITE_BACK_TO_INCIDENT_FAIL("定期回写同一地址库id至警情失败"),
        KAFKA_CONSUME_FAIL("消费kafka数据失败"),
        UPDATE_TOKEN_CATCH_FAIL("更新token缓存失败"),
        GET_TOKEN_CATCH_FAIL("获取token缓存失败"),
        FIND_CTI_CALL_FAIL("获取呼入记录失败"),
        SAVE_BATUNIT_FAIL("保存或修改作战单元失败"),
        FIND_BATUNIT_FAIL("查询作战单元失败"),
        SAVE_DISPATCHPLANRECORD_FAIL("保存调派方案使用记录失败"),

        TTANSFORM_GRADEJUDGE_FAILE("调用智能推荐等级服务失败"),
        TTANSFORM_POWERTRANSFER_FAILE("调用智能推荐力量服务失败"),
        RECOMMEND_VEHICLE_FAIL("智能推荐调派车辆失败"),
        FIND_SMARTRECOMMEND_FAIL("查询智能辅助记录失败"),
