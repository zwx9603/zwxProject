...
@Component("leaderScheduleServiceImpl")
public class LeaderScheduleServiceImpl implements LeaderScheduleService{
    private static final Logger logger = LoggerFactory.getLogger(LeaderScheduleServiceImpl.class);
    ...
    @Autowired
    public LeaderScheduleServiceImpl(LogService logService , Environment env,
...
//                            DateUtils.add(new Date(lastUpdataTime), fieldType, onceTime).getTime();
                    ...
                    //单次查询
                    List<LeaderEntity> list = findScheduleTable(lastUpdataTime, endTime);
                    //结果不为空 保存数据
...
//                    accessor.save(entity);
    ...
    private static String sql = " select " +
            ...
            "yxx," +
            "sjc  " +
            "  from  v_td_t_coc_fire_ld schedule" +
     ...
     * @return 返回查询结果
     */
    private List<LeaderEntity> findScheduleTable(Long startTime , Long endTime ) {
        PreparedStatement pstat = null;
        ResultSet rs = null;
        List<LeaderEntity> list ;
        try ( Connection conn = connectionsFactory.getConnection( env.getProperty("leader") ) ) {
            if (startTime < endTime) {
                ...
                while (rs.next()) {
                    ...
                    //一般属性
                    entity.setOfficeNumber( rs.getString("bgdh") );
                    entity.setUnitName( rs.getString("dwmc") );
                    entity.setLeaderName( rs.getString("xm") );
                    entity.setMobliPhone( rs.getString("yddh") );
                    entity.setDuty( rs.getString("zw") );
