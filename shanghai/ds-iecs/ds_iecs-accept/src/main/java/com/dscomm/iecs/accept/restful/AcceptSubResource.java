...
import com.dscomm.iecs.accept.graphql.inputbean.SendWebsocketMessageInputInfo;
...
public class AcceptSubResource {
    ...
    public AcceptSubResource(LogService logService, SessionDataStore sessionDataStore, ServletService servletService,
                             AuthenticateService authenticateService, LocationService locationService, IncidentInformationService incidentInformationService, WebSocketPushService webSocketPushService) {
...
//            authenticateService.subAuthenticate(tokenkey);
    ...
    @Path("sendWebsocketMessage")
    @POST
    public DataVO<Boolean> sendWebsocketMessage(SendWebsocketMessageInputInfo param){
        try {
            if (param==null||param.getReceivers()==null||param.getReceivers().isEmpty()){
                return new DataVO<>(false);
            }
            if (StringUtils.isBlank(param.getReceiverType())||!"user,businessOrg.role".contains(param.getReceiverType())){
                param.setReceiverType("user");
            }
            List<ReceiverMessageBean> list=new ArrayList<>();
            Set<String> set=new HashSet<>();
            set.addAll(param.getReceivers());
            for (String s : set) {
                ReceiverMessageBean receiver=new ReceiverMessageBean(s,param.getSystem(),param.getReceiverType());
                list.add(receiver);
            }
            webSocketPushService.pushMessage(param.getCode(),param.getMsg(),list);
            return new DataVO<>(true);
        }catch (Exception ex){
            logService.erorLog(logger,"rest","sendWebsocketMessage","fail to send message",ex);
            return new DataVO<>(false);
        }
    }
}
