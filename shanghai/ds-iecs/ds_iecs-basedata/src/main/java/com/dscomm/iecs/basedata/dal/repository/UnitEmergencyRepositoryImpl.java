 ...
 */
@Repository
public class UnitEmergencyRepositoryImpl {
    @PersistenceContext
    private EntityManager entityManager;

    @Transactional(readOnly = true)
    public List<UnitEmergencyEntity> findUnitEmergencyCondition(String keyword,
                                                                String districtCode,
                                                                String squadronOrganizationSearchPath,
                                                                Boolean whetherPage, int page, int size){
        String headsql = "  select station  from  UnitEmergencyEntity station   where   station.valid = 1   ";
        ...
        if (Strings.isNotBlank(keyword)) {
            conditionsql = conditionsql + " and ( station.unitName  like  :GJZ  or station.unitAddress  like  :GJZ  ) ";
        }
    ...
    @Transactional(readOnly = true)
    public Integer findUnitEmergencyTotal(String keyword, String districtCode, String squadronOrganizationSearchPath){
        String headsql = "  select count(1) as NUM  from   UnitEmergencyEntity station   where   station.valid = 1   ";
