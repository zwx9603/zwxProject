...
import com.dscomm.iecs.basedata.dal.po.SystemRoleEntity;
 ...
 *
 */
@Component("systemRoleScheduleServiceImpl")
public class SystemRoleScheduleServiceImpl implements SystemRoleScheduleService {


    private static final Logger logger = LoggerFactory.getLogger(SystemRoleScheduleServiceImpl.class);
    ...
    @Autowired
    public SystemRoleScheduleServiceImpl(LogService logService , Environment env,
    ...
    public void transport(Long targetTime) {
        try {
            logService.infoLog(logger, "service", "systemRole", "service is started...");
            Long logStart = System.currentTimeMillis();

            TransporterEntity entity = accessor.getById("jcj_bd_xtjs", TransporterEntity.class);
            if (entity == null) {
                logService.erorLog(logger, "service", "systemRole", "table config not exist ",  new Exception() );
                return ;
            }
            ...
            if (lastUpdataTime != null && targetTime != null && lastUpdataTime < targetTime) {
                ...
                while (lastUpdataTime < targetTime && i< 7 ) {
                    ...
                    //单次查询
                    List<SystemRoleEntity> list = findScheduleTable(lastUpdataTime, endTime);
                    //结果不为空 保存数据
            ...
            logService.infoLog(logger, "service", "systemRole", String.format("service is finished,execute time is :%sms", logEnd - logStart));
        } catch (Exception ex) {
            logService.erorLog(logger, "service", "systemRole", String.format(" transport account task fail  , target time :%s.", new Timestamp(targetTime) ), ex);
    ...
    private static String sql = " select " +
            ...
            " jsms , " +
            " '' as  bz  " +
            "  from  v_td_jcj_bd_xtjs schedule where schedule.sjc >?  and schedule.sjc <= ?  ";
     ...
     * @return 返回查询结果
     */
    private List<SystemRoleEntity> findScheduleTable( Long startTime , Long endTime ) {
        PreparedStatement pstat = null;
        ResultSet rs = null;
        List<SystemRoleEntity> list ;
        try ( Connection conn = connectionsFactory.getConnection( env.getProperty("systemRole") ) ) {
            if (startTime < endTime) {
                ...
                while (rs.next()) {
                    ...
                    //一般属性
                    entity.setRoleCode( rs.getString("bh") );
                    entity.setName( rs.getString("mc") );
                    entity.setRoleDescribe( rs.getString("jsms") );
    ...
    public  void  transportDelete() {
            try{
                //移除数据集合
                List<SystemRoleEntity> removeList = new ArrayList<>() ;
                //获得全部业务角色
                 List<SystemRoleEntity> allRoles = accessor.list( SystemRoleEntity.class ) ;
                //获得原数据库 全部业务角色 唯一标识
                List<String> ids = findScheduleTableByDelete() ;
                if( ids == null || ids.size() < 1 ){
                    removeList.addAll( allRoles ) ;
                }else {

                    if( allRoles != null && allRoles.size() > 0   ){
                        for( SystemRoleEntity role :  allRoles ){
                            String bid = role.getId() ;
                            if( !ids.contains( bid )){
                                removeList.add( role ) ;
                            }
                        }
    ...
    private static String sqldelete = " select " +
            " id   " +
            "  from  v_td_jcj_bd_xtjs schedule  ";
